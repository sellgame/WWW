<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:52:"U:\phpStudy_A\WWW/search/index\view\index\plist.html";i:1488158111;s:54:"U:\phpStudy_A\WWW/search/index\view\public\header.html";i:1488156917;s:55:"U:\phpStudy_A\WWW/search/index\view\Public\sidebar.html";i:1488156923;s:54:"U:\phpStudy_A\WWW/search/index\view\Public\footer.html";i:1488156911;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
...
<body>
    ...
    <div class="am-g">
        <div class="am-u-lg-8">
            <div id="tip">相关匹配文献 <span id="counts"></span> 篇 (<span class="am-kai" id="setime">本次搜索用时 0s</span>) &nbsp;&nbsp;&nbsp;&nbsp;<span id="download"></span></div>
...
<script src="<?php echo \think\Config::get('js_path'); ?>amazeui.min.js"></script>
    ...
    function searchs(cpage) {
        $.AMUI.progress.start();
        $.post("<?php echo Url('publication/plist'); ?>", {
            keys: $('#keys').val(),
            sid: $('#sid').val(),
            year: $('#year').val(),
            issue: $('#issue').val(),
            ...
            if (status == 'success') {
                ...
                if (data.counts < 1000 && $('#issue').val() != '') {
                    $('#download').html('[<a href="<?php echo Url('index/downpdf'); ?>?sid=' + $('#sid').val() + '&year=' + $('#year').val() + '&issue=' + $('#issue').val() + '" target="_blank">下载本期全部全文</a>]');
                }
                //重新初始化折叠事件
                $.AMUI.accordion.init();
                //重置分页属性
