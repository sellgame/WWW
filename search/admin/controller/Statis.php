...
class statis extends \think\Controller
{
	...
	/**
	 *下载数据统计
	 * @param uid 用户id type 查询类型 day天 month月 year年
	 * @return 
	 * @throws 
	*/
	...
	*/
	public function kvist(){
		if(Request::instance()->isAjax()){
			$uid = input('post.uid');
			if(!empty($uid)){
				$res = Db::connect('userdb_config')->name('visit_k')->field('distinct keyword')->where('uid','=',$uid)->limit(10)->order('visittime DESC')->select();
			} else {
				$res = Db::connect('userdb_config')->name('visit_k')->field('keyword')->limit(10)->order('visittime DESC')->select();
			}
	...
	*/
	public function avist(){
		if(Request::instance()->isAjax()){
			$uid = input('post.uid');
			if(!empty($uid)){
				$res = Db::connect('userdb_config')->name('visit_a')->field('distinct docid')->where('uid','=',$uid)->limit(10)->order('visittime DESC')->select();
			} else{
				$res = Db::connect('userdb_config')->name('visit_a')->field('distinct docid')->limit(10)->order('visittime DESC')->select();
			}
			if(count($res)>0){
				foreach($res as $id=>$vo){
